...
class PostgresQuery(BaseQuery):
    ...
    Uses %s placeholders (psycopg2 standard). All field names are validated
    ...
    def search(self, field, value):
        if field and value:
            safe_field = self._validate_field_name(field)
            search_string = f"{safe_field} ILIKE %s"
            self.params.append(f"%{self._escape_like(value)}%")
    ...
    def count(self):
        ...
        query = f"SELECT COUNT(*) as query_count FROM ({count_query}) AS t"
